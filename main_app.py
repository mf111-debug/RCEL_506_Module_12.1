...
def load_data():
    ...
    info_cols = ["station_id", "lat", "lon", "capacity", "name"]
    status_cols = [
        "station_id",
        "num_bikes_available",
        "num_bikes_disabled",
    ...
    df["num_bikes_available"] = df["num_bikes_available"].fillna(0)
    df["num_bikes_disabled"] = df["num_bikes_disabled"].fillna(0)
    df["num_docks_available"] = df["num_docks_available"].fillna(0)
    df["num_docks_disabled"] = df["num_docks_disabled"].fillna(0)
...
def bike_share_system_cdmx_plot(dataframe, station_number=None, mode_choice="Find Bikes", cluster=True):
    ...
    ">
        <b style="color: black;">Marker Legend</b><br>
        <span style="color: black;">
            <i style="background: green; width: 10px; height: 10px; display: inline-block; margin-right: 6px;"></i> High
        </span><br>
        <span style="color: black;">
            <i style="background: orange; width: 10px; height: 10px; display: inline-block; margin-right: 6px;"></i> Medium
        </span><br>
        <span style="color: black;">
            <i style="background: red; width: 10px; height: 10px; display: inline-block; margin-right: 6px;"></i> Low
        </span><br>
        <span style="color: black;">
            <i style="background: blue; width: 10px; height: 10px; display: inline-block; margin-right: 6px;"></i> Selected
        </span>
    </div>
    """
...
with kpi3:
    st.metric(
        ...
        int(filtered_df["num_bikes_available"].sum()) if not filtered_df.empty else 0
    )

with kpi4:
    st.metric(
        "Total Docks Available",
        int(filtered_df["num_docks_available"].sum()) if not filtered_df.empty else 0
    )
...
with col1:
    ...
    if filtered_df.empty:
        ...
        selected_label = st.selectbox("Choose a station", selector_labels)
...
with chart1:
    st.subheader("Top 20 Stations by Bikes Available")
    if not filtered_df.empty:
        bikes_chart_df = (
        ...
        st.bar_chart(bikes_chart_df)

with chart2:
    st.subheader("Top 20 Stations by Docks Available")
    if not filtered_df.empty:
        docks_chart_df = (
